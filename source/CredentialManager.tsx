...
export const CredentialManager: React.FC<CredentialManagerProps> = ({
  ...
  const [coreCredentialManager, setCoreCredentialManager] = useState<CoreCredentialManager | null>(null);
  ...
  const initializeCredentialManager = async () => {
    ...
    try {
      ...
      manager.on('credentialRotated', (provider: string, oldKeyId: string, newKeyId: string) => {
        console.log(`Credential rotated for ${provider}: ${oldKeyId} -> ${newKeyId}`);
        loadCredentials(manager);
      });
  ...
  const loadCredentials = async (manager: CoreCredentialManager) => {
    try {
      ...
      const credentialEntries: CredentialEntry[] = providers.map(provider => ({
        id: provider,
  ...
  const handleAddCredential = async () => {
    ...
    try {
      ...
      await coreCredentialManager.storeCredential(newCredential.provider, newCredential.value);
      ...
      const change: ConfigurationChange = {
        ...
        newValue: { provider: newCredential.provider, type: newCredential.type },
        timestamp: new Date(),
  ...
  const handleRemoveCredential = async (providerId: string) => {
    ...
    const confirm = window.confirm(`Are you sure you want to remove credentials for ${providerId}?`);
    if (!confirm) return;
  ...
  const handleTestCredential = async (providerId: string) => {
    ...
    try {
      ...
      if (credential) {
        alert(`✓ Credential for ${providerId} is accessible (length: ${credential.length})`);
      }
    } catch (error) {
  ...
  const validateCredential = (credential: CredentialFormData): ValidationError[] => {
    ...
    if (!credential.value.trim()) {
      errors.push({ field: 'value', message: 'Credential value is required', severity: 'error' });
    } else {
      // Basic validation for API keys
      if (credential.type === 'api_key') {
        if (credential.provider === 'anthropic' && !credential.value.startsWith('sk-ant-')) {
          errors.push({ field: 'value', message: 'Anthropic API keys should start with "sk-ant-"', severity: 'warning' });
        }
        if (credential.provider === 'openai' && !credential.value.startsWith('sk-')) {
          errors.push({ field: 'value', message: 'OpenAI API keys should start with "sk-"', severity: 'warning' });
        }
        if (credential.value.length < 20) {
          errors.push({ field: 'value', message: 'API key seems too short', severity: 'warning' });
        }
      }
    }
  ...
  if (!isInitialized && coreCredentialManager) {
    return (
      <div style={{ padding: '16px' }}>
        <h2 style={{ color: '#569cd6', marginBottom: '16px' }}>Initialize Credential Manager</h2>
        ...
        <div style={{ marginTop: '16px', fontSize: '12px', color: '#808080' }}>
          <p>The master password is used to encrypt/decrypt stored credentials.</p>
    ...
    <div style={{ padding: '16px' }}>
      ...
      {validationErrors.length > 0 && (
        <div style={{ marginBottom: '16px' }}>
          {validationErrors.map((error, index) => (
            <div key={index} style={{
              padding: '8px',
              backgroundColor: error.severity === 'error' ? '#3c1e1e' : '#3c3c1e',
              border: `1px solid ${error.severity === 'error' ? '#f44747' : '#ffcc02'}`,
              borderRadius: '4px',
              marginBottom: '4px',
              fontSize: '12px'
            }}>
              <span style={{ color: error.severity === 'error' ? '#f44747' : '#ffcc02' }}>
      ...
      {credentialStats && (
        <div style={{
          ...
          <h3 style={{ color: '#569cd6', margin: '0 0 8px 0', fontSize: '14px' }}>Security Status</h3>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '8px', fontSize: '12px' }}>
            <div>
              ...
              <span style={{ color: '#4fc1ff' }}>{credentialStats.totalProviders}</span>
            </div>
            <div>
              <span style={{ color: '#cccccc' }}>Encryption: </span>
              <span style={{ color: '#4fc1ff' }}>{credentialStats.encryptionAlgorithm}</span>
            </div>
            <div>
              <span style={{ color: '#cccccc' }}>Last Updated: </span>
              <span style={{ color: '#4fc1ff' }}>{credentialStats.lastUpdated.toLocaleDateString()}</span>
            </div>
            <div>
              <span style={{ color: '#cccccc' }}>Rotation: </span>
              <span style={{ color: credentialStats.rotationEnabled ? '#4fc1ff' : '#808080' }}>
      ...
      {/* Credentials List */}
      <div style={{ marginBottom: '16px' }}>
        <h3 style={{ color: '#569cd6', marginBottom: '12px', fontSize: '14px' }}>Stored Credentials</h3>
...
const NewCredentialForm: React.FC<NewCredentialFormProps> = ({
  ...
  return (
    <div style={{
      ...
      <h3 style={{ color: '#569cd6', margin: '0 0 16px 0', fontSize: '14px' }}>Add New Credential</h3>
        ...
        <div>
          ...
          <select
            ...
            style={{
              ...
              backgroundColor: '#3c3c3c',
              color: '#d4d4d4',
              border: `1px solid ${getFieldError('provider') ? '#f44747' : '#5a5a5a'}`,
              borderRadius: '3px',
              fontSize: '12px'
            }}
          >
          ...
          {getFieldError('provider') && (
            <div style={{ color: '#f44747', fontSize: '10px', marginTop: '2px' }}>
      ...
      <div style={{ marginBottom: '16px' }}>
        ...
        <input
          ...
          style={{
            ...
            backgroundColor: '#3c3c3c',
            color: '#d4d4d4',
            border: `1px solid ${getFieldError('value') ? '#f44747' : '#5a5a5a'}`,
            borderRadius: '3px',
            fontSize: '12px'
          }}
        />
        {getFieldError('value') && (
          <div style={{ 
            color: getFieldError('value')?.severity === 'error' ? '#f44747' : '#ffcc02', 
            fontSize: '10px', 
            marginTop: '2px' 
          }}>
