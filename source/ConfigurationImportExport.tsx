...
export const ConfigurationImportExport: React.FC<ConfigurationImportExportProps> = ({
  context,
  onChange
}) => {
  ...
  const availableComponents = [
    { id: 'agents', name: 'Agent Configurations', description: 'All agent definitions and tools' },
    { id: 'models', name: 'Model Selection', description: 'Model configuration and selection rules' },
    { id: 'security', name: 'Security Settings', description: 'Access patterns and security configuration (excluding credentials)' },
    { id: 'logging', name: 'Logging Configuration', description: 'Logging levels and output settings' },
    { id: 'orchestration', name: 'Orchestration Settings', description: 'Core orchestration and permission settings' }
  ];

  const handleExport = async () => {
    setIsProcessing(true);
    try {
      const configToExport = filterConfigForExport(context.resolvedConfig, exportOptions);
      const formatted = await formatConfiguration(configToExport, exportOptions);
  ...
  const processImportContent = async (content: string, filename: string): Promise<ImportResult> => {
    try {
      // Determine format from file extension
      const format = filename.endsWith('.yaml') || filename.endsWith('.yml') ? 'yaml' :
  ...
  return (
    <div style={{ padding: '16px' }}>
      ...
      <div style={{ marginBottom: '16px' }}>
        <h2 style={{ color: '#569cd6', marginBottom: '12px' }}>Import/Export Configuration</h2>
        ...
        <div style={{ display: 'flex', gap: '4px' }}>
          {[
            ...
            <button
              ...
              style={{
                padding: '6px 12px',
                backgroundColor: activeTab === tab.id ? '#0e639c' : 'transparent',
      ...
      {activeTab === 'export' && (
        <div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            {/* Export Options */}
            <div>
              <h3 style={{ color: '#569cd6', marginBottom: '12px', fontSize: '14px' }}>Export Options</h3>
              ...
              <div style={{ marginBottom: '12px' }}>
                <label style={{ display: 'block', marginBottom: '8px', fontSize: '12px', color: '#cccccc' }}>
                  Components to Export
                </label>
                {availableComponents.map(component => (
                  <label key={component.id} style={{
                    display: 'flex',
                    alignItems: 'center',
                    marginBottom: '4px',
                    fontSize: '11px',
                    ...
                    <input
                      type="checkbox"
                      checked={exportOptions.selectedComponents.includes(component.id)}
                      onChange={(e) => {
                        const selected = e.target.checked
                          ? [...exportOptions.selectedComponents, component.id]
                          : exportOptions.selectedComponents.filter(id => id !== component.id);
                        setExportOptions(prev => ({ ...prev, selectedComponents: selected }));
                      }}
                      style={{ marginRight: '6px' }}
                    />
                    <div>
                      <div>{component.name}</div>
                      <div style={{ fontSize: '10px', color: '#808080' }}>{component.description}</div>
                    </div>
                  </label>
                ))}
              </div>
              ...
              <div style={{ marginBottom: '12px' }}>
                <label style={{ display: 'flex', alignItems: 'center', fontSize: '11px', color: '#cccccc', marginBottom: '4px' }}>
                  <input
                    type="checkbox"
                    checked={exportOptions.includeUserSettings}
                    onChange={(e) => setExportOptions(prev => ({ ...prev, includeUserSettings: e.target.checked }))}
                    style={{ marginRight: '6px' }}
                  />
                ...
                <label style={{ display: 'flex', alignItems: 'center', fontSize: '11px', color: '#f44747' }}>
                  <input
                    type="checkbox"
            ...
            {/* Export Preview */}
            <div>
              <h3 style={{ color: '#569cd6', marginBottom: '12px', fontSize: '14px' }}>Export Preview</h3>
              <textarea
                value={exportResult}
                readOnly
                style={{
      ...
      {activeTab === 'import' && (
        <div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            {/* Import Area */}
            <div>
              <h3 style={{ color: '#569cd6', marginBottom: '12px', fontSize: '14px' }}>Import Configuration</h3>
              ...
              >
                <div style={{ fontSize: '24px', marginBottom: '8px', color: '#569cd6' }}>📁</div>
            ...
            {/* Import Results */}
            <div>
              <h3 style={{ color: '#569cd6', marginBottom: '12px', fontSize: '14px' }}>Import Results</h3>
              ...
              {importResult ? (
                <div>
                  ...
                  <div style={{
                    padding: '8px',
                    backgroundColor: importResult.success ? '#1e3a1e' : '#3c1e1e',
                    border: `1px solid ${importResult.success ? '#4fc1ff' : '#f44747'}`,
                    borderRadius: '4px',
                    marginBottom: '12px',
                    fontSize: '12px'
                  }}>
                  ...
                  {/* Errors and Warnings */}
                  {[...importResult.errors, ...importResult.warnings].map((issue, index) => (
                    <div key={index} style={{
                      padding: '6px',
                      backgroundColor: issue.severity === 'error' ? '#3c1e1e' : '#3c3c1e',
                      border: `1px solid ${issue.severity === 'error' ? '#f44747' : '#ffcc02'}`,
                      borderRadius: '3px',
                      marginBottom: '4px',
                      fontSize: '11px'
                    }}>
                      <span style={{ color: issue.severity === 'error' ? '#f44747' : '#ffcc02' }}>
      ...
      {activeTab === 'presets' && (
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
            <h3 style={{ color: '#569cd6', margin: 0, fontSize: '14px' }}>Configuration Presets</h3>
            <button
          ...
          <div style={{ space: '8px' }}>
            {presets.length === 0 ? (
              <div style={{
                ...
                No presets saved yet. Save your current configuration as a preset to reuse it later.
              </div>
            ) : (
              presets.map(preset => (
                <PresetCard
...
const PresetCard: React.FC<PresetCardProps> = ({ preset, onLoad, onDelete }) => (
  <div style={{
    padding: '12px',
    backgroundColor: '#252526',
    border: '1px solid #3c3c3c',
    ...
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
      <div style={{ flex: 1 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
          <span style={{ color: '#569cd6', fontWeight: 'bold', fontSize: '14px' }}>
            {preset.name}
          </span>
...
const NewPresetForm: React.FC<NewPresetFormProps> = ({ preset, onChange, onSave, onCancel }) => (
  <div style={{
    position: 'fixed',
    top: 0,
    left: 0,
...
function filterConfigForExport(config: any, options: ExportOptions): any {
  ...
  if (options.selectedComponents.includes('orchestration')) {
    if (config.defaultPermissions) filtered.defaultPermissions = config.defaultPermissions;
...
async function formatConfiguration(config: any, options: ExportOptions): Promise<string> {
...
function convertToYaml(obj: any, includeComments: boolean): string {
  ...
  let yaml = includeComments ? '# AI Code Configuration Export\n# Generated on ' + new Date().toISOString() + '\n\n' : '';
...
function formatAsTypeScript(config: any, includeComments: boolean): string {
  let ts = includeComments ? `// AI Code Configuration Export
// Generated on ${new Date().toISOString()}
...
` : '';
  ...
  ts += `export const configuration: CompleteConfig = ${JSON.stringify(config, null, 2)};`;
  return ts;
}
