...
export const AgentStatusDisplay: React.FC<AgentStatusDisplayProps> = ({ 
  ...
  const { agents, activeOperations, isConnected, stats } = useAgentCommunication();
  ...
  useInput((input, key) => {
    ...
    if (key.downArrow && selectedAgentIndex < Math.min(agents.length - 1, maxAgents - 1)) {
      setSelectedAgentIndex(selectedAgentIndex + 1);
    }
  ...
  const getAgentActivityStatus = (agent: AgentCapability): 'active' | 'idle' | 'busy' => {
    const agentOperations = Array.from(activeOperations.values())
      .filter(op => op.requestingAgent === agent.id);
  ...
  if (!isConnected) {
    return (
      <Box flexDirection="column" borderStyle="round" borderColor="red" padding={1}>
        <Text color="red" bold>Agent System Disconnected</Text>
  ...
  if (agents.length === 0) {
    return (
      <Box flexDirection="column" borderStyle="round" borderColor="yellow" padding={1}>
        <Text color="yellow" bold>No Agents Registered</Text>
  ...
  if (compact) {
    return (
      <Box flexDirection="column" borderStyle="single" borderColor="cyan" padding={1}>
        <Text color="cyan" bold>Agents ({stats.activeAgents})</Text>
    ...
    <Box flexDirection="column" borderStyle="round" borderColor="cyan" padding={1}>
      ...
      {showDetails && selectedAgent && (
        <Box marginTop={1} borderStyle="single" borderColor="gray" padding={1}>
          <Text color="cyan" bold>{selectedAgent.name} Details</Text>
          <Text dimColor>ID: {selectedAgent.id}</Text>
          <Text>{selectedAgent.description}</Text>
          
          {showTools && selectedAgent.tools && selectedAgent.tools.length > 0 && (
            <Box marginTop={1} flexDirection="column">
              <Text color="yellow" bold>Tools:</Text>
              {selectedAgent.tools.map((tool, idx) => (
                <Text key={idx} dimColor>
                  • {tool.name || tool.id}: {tool.description || 'No description'}
                </Text>
              ))}
            </Box>
          )}
          ...
          {selectedAgent.endpoints && selectedAgent.endpoints.length > 0 && (
            <Box marginTop={1} flexDirection="column">
              <Text color="green" bold>Endpoints:</Text>
      ...
      {stats.activeAgents > maxAgents && (
        <Box marginTop={1}>
          <Text dimColor>... and {stats.activeAgents - maxAgents} more agents</Text>
        </Box>
      )}
    </Box>
  );
...
export const AgentSummary: React.FC<AgentSummaryProps> = ({ agentId }) => {
  const { agents, getAgentStatus, activeOperations, recentResponses } = useAgentCommunication();
  ...
  const agentOps = Array.from(activeOperations.values())
    .filter(op => op.requestingAgent === agent.id);
  ...
  return (
    <Box flexDirection="column" borderStyle="single" borderColor="cyan" padding={1}>
      <Text color="cyan" bold>{agent.name} Summary</Text>
      ...
      {agentResponses.length > 0 && (
        <Box marginTop={1} flexDirection="column">
          <Text color="green" bold>Recent Responses:</Text>
