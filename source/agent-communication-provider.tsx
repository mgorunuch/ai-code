...
interface AgentCommunicationContextType extends AgentCommunicationState {
	...
	sendRequest: (
		request: Omit<OperationRequest, 'requestId'>
...
const AgentCommunicationContext =
	createContext<AgentCommunicationContextType | null>(null);

export const useAgentCommunication = () => {
	const context = useContext(AgentCommunicationContext);
	if (!context) {
		throw new Error(
			'useAgentCommunication must be used within an AgentCommunicationProvider'
		);
	}
	return context;
};
...
export const AgentCommunicationProvider: React.FC<
	AgentCommunicationProviderProps
> = ({ children, autoInit = false }) => {
	...
	const setupEventListeners = (orchestrator: CoreOrchestrator) => {
		...
		orchestrator.on('requestReceived', (request: OperationRequest) => {
			setState((prev) => ({
				...prev,
				activeOperations: new Map(prev.activeOperations).set(
					request.requestId,
					request
				),
		...
		orchestrator.on('requestCompleted', (response: OperationResponse) => {
			setState((prev) => {
				...
				return {
					...
					stats: {
						...prev.stats,
						totalResponses: prev.stats.totalResponses + 1
					}
				};
			});
		});
		...
		orchestrator.on(
			'requestFailed',
			(request: OperationRequest, error: Error) => {
				setState((prev) => {
					const newActiveOps = new Map(prev.activeOperations);
					...
					const newResponses = [errorResponse, ...prev.recentResponses].slice(
						0,
						50
					);
		...
		orchestrator.on('agentUnregistered', (agentId: AgentId) => {
			setState((prev) => ({
				...
				stats: {
					...prev.stats,
					activeAgents: Math.max(0, prev.stats.activeAgents - 1)
				}
			}));
		});
		...
		orchestrator.on(
			'modelSelected',
			(criteria: ModelSelectionCriteria, result: ModelSelectionResult) => {
				setState((prev) => ({
					...prev,
					modelSelections: [
						{ criteria, result },
		...
		if (commSystem) {
			...
			commSystem.on(
				'questionAsked',
				(from: AgentId, to: AgentId, question: QuestionRequest) => {
					setState((prev) => ({
						...prev,
						questionHistory: [
							{ from, to, question },
			...
			commSystem.on(
				'questionAnswered',
				(from: AgentId, to: AgentId, response: QuestionResponse) => {
					setState((prev) => {
						const newHistory = [...prev.questionHistory];
						const questionIndex = newHistory.findIndex(
							(q) => q.from === from && q.to === to && !q.response
						);
						if (questionIndex >= 0) {
	...
	const askQuestion = async (
		...
		return await state.orchestrator.askQuestion(
			fromAgent,
			question,
			targetAgent
		);
