...
export const MessageQueue: React.FC<MessageQueueProps> = ({ 
  ...
  const { messageHistory, questionHistory, isConnected } = useAgentCommunication();
  ...
  useInput((input, key) => {
    ...
    if (key.downArrow && selectedIndex < Math.min(displayMessages.length - 1, maxMessages - 1)) {
      setSelectedIndex(selectedIndex + 1);
    }
  ...
  const filteredMessages = filterByAgent 
    ? messageHistory.filter(msg => msg.from === filterByAgent || msg.to === filterByAgent)
    : messageHistory;
  ...
  const getMessageTypeColor = (endpoint: string): string => {
    switch (endpoint) {
      ...
      case 'transform': return 'magenta';
      case 'handle': return 'green';
  ...
  if (!isConnected) {
    return (
      <Box flexDirection="column" borderStyle="round" borderColor="red" padding={1}>
        <Text color="red" bold>Message Queue Unavailable</Text>
    ...
    <Box flexDirection="column" borderStyle="round" borderColor="green" padding={1}>
      <Box flexDirection="row" justifyContent="space-between">
        ...
        <Box flexDirection="row" gap={1}>
          ...
          <Text dimColor>
            ↑↓: Navigate • Enter: Details • P: Pause
          </Text>
        </Box>
      </Box>
      ...
      {displayMessages.length === 0 ? (
        ...
        <Box marginTop={1} flexDirection="column">
          ...
          {displayMessages.map((message, index) => {
            ...
            return (
              <Box key={message.messageId} flexDirection="row" justifyContent="space-between">
                {showTimestamp && (
                  <Box width={10}>
                    <Text 
                ...
                <Box width={20}>
                  <Text 
                    ...
                    wrap="truncate"
                  >
                    {isSelected ? '→ ' : '  '}{message.from} → {message.to}
                  </Text>
                </Box>
      ...
      {showDetails && selectedMessage && (
        <Box marginTop={1} borderStyle="single" borderColor="gray" padding={1}>
          <Text color="cyan" bold>Message Details</Text>
          <Text dimColor>ID: {selectedMessage.messageId}</Text>
          <Text dimColor>Time: {selectedMessage.timestamp.toISOString()}</Text>
          <Text>From: <Text color="yellow">{selectedMessage.from}</Text></Text>
          <Text>To: <Text color="yellow">{selectedMessage.to}</Text></Text>
          <Text>Endpoint: <Text color={getMessageTypeColor(selectedMessage.endpoint)}>{selectedMessage.endpoint}</Text></Text>
          
          <Box marginTop={1} flexDirection="column">
            <Text color="green" bold>Payload:</Text>
      ...
      {filteredMessages.length > maxMessages && (
        <Box marginTop={1}>
          <Text dimColor>... and {filteredMessages.length - maxMessages} more messages</Text>
        </Box>
      )}
    </Box>
  );
...
export const QuestionQueue: React.FC<QuestionQueueProps> = ({ 
  ...
  useInput((input, key) => {
    ...
    if (key.downArrow && selectedIndex < Math.min(questionHistory.length - 1, maxQuestions - 1)) {
      setSelectedIndex(selectedIndex + 1);
    }
  });
  ...
  if (!isConnected) {
    return (
      <Box flexDirection="column" borderStyle="round" borderColor="red" padding={1}>
        <Text color="red" bold>Question Queue Unavailable</Text>
        <Text dimColor>Agent communication system not connected</Text>
      </Box>
    );
  }
  ...
  return (
    <Box flexDirection="column" borderStyle="single" borderColor="magenta" padding={1}>
      <Text color="magenta" bold>Question Queue</Text>
      ...
      {displayQuestions.length === 0 ? (
        ...
        <Box marginTop={1} flexDirection="column">
          {displayQuestions.map((entry, index) => {
            ...
            return (
              <Box key={index} marginBottom={1} borderStyle="single" borderColor={hasAnswer ? 'green' : 'yellow'} padding={1}>
                <Box flexDirection="row" justifyContent="space-between">
                  <Text 
                    color={isSelected ? 'cyan' : 'white'} 
                    bold={isSelected}
                  >
                    {isSelected ? '→ ' : '  '}{entry.from} asks {entry.to}
                  </Text>
      ...
      {questionHistory.length > maxQuestions && (
        <Box marginTop={1}>
          <Text dimColor>... and {questionHistory.length - maxQuestions} more questions</Text>
        </Box>
      )}
    </Box>
  );
