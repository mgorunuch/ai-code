...
import type { AgentId, QuestionRequest, OperationRequest } from './core/types.js';

interface AgentInteractionPanelProps {
  onClose?: () => void;
}

export const AgentInteractionPanel: React.FC<AgentInteractionPanelProps> = ({ onClose }) => {
  const { agents, sendRequest, askQuestion, isConnected } = useAgentCommunication();
  const [mode, setMode] = useState<'menu' | 'ask-question' | 'send-operation' | 'select-agent'>('menu');
  const [selectedAgentIndex, setSelectedAgentIndex] = useState(0);
  const [inputText, setInputText] = useState('');
  const [operationType, setOperationType] = useState<OperationType>(OperationType.QUESTION);
  ...
  if (!isConnected) {
    return (
      <Box flexDirection="column" borderStyle="round" borderColor="red" padding={1}>
        <Text color="red" bold>Agent Interaction Unavailable</Text>
        <Text dimColor>Initialize the orchestrator to interact with agents</Text>
        <Box marginTop={1}>
  ...
  if (isExecuting) {
    return (
      <Box flexDirection="column" borderStyle="round" borderColor="yellow" padding={1}>
        <Text color="yellow" bold>Executing...</Text>
  ...
  return (
    <Box flexDirection="column" borderStyle="round" borderColor="cyan" padding={1}>
      <Text color="cyan" bold>Agent Interaction Panel</Text>
      ...
      {lastResult && (
        <Box marginTop={1} borderStyle="single" borderColor="green" padding={1}>
          <Text color="green" bold>Last Result:</Text>
          <Text>{JSON.stringify(lastResult, null, 2)}</Text>
        </Box>
      )}

      {mode === 'menu' && (
        <MenuView agents={agents} />
      )}
  ...
  <Box flexDirection="column" marginTop={1}>
    ...
    <Box marginTop={1}>
      ...
      {agents.map((agent, idx) => (
        <Text key={agent.id} dimColor>• {agent.name} ({agent.id})</Text>
      ))}
    </Box>
...
const AskQuestionView: React.FC<{
  ...
  <Box flexDirection="column" marginTop={1}>
    <Text color="yellow" bold>Ask Question to Agent</Text>
...
const SendOperationView: React.FC<{
  ...
  <Box flexDirection="column" marginTop={1}>
    <Text color="green" bold>Send Operation to Agent</Text>
    
    <Box marginTop={1}>
      <Text>Operation Type: <Text color="yellow">{operationType}</Text></Text>
    </Box>
...
const SelectAgentView: React.FC<{
  ...
  <Box flexDirection="column" marginTop={1}>
    <Text color="cyan" bold>Select Target Agent</Text>
...
export const QuickActions: React.FC<QuickActionsProps> = ({ onQuestionClick, onOperationClick }) => {
