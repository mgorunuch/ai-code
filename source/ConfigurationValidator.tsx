...
import type {
	...
	ConfigurationSuggestion as CoreConfigurationSuggestion
...
export const ConfigurationValidator: React.FC<ConfigurationValidatorProps> = ({
	...
	const [coreValidator, setCoreValidator] =
	...
	const validateConfiguration = async () => {
		...
		try {
			// Use core validator
			const coreResult = await coreValidator.validateConfiguration(
				targetConfig as CompleteConfig
			);
		...
		<div style={{ padding: '16px' }}>
			...
			>
				...
				<div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
					{isValidating && (
						<span style={{ color: '#4fc1ff', fontSize: '12px' }}>
							Validating...
						</span>
					)}
					<button
			...
			{validationSummary && (
				<div
					style={{
						...
						border: `1px solid ${
							validationSummary.valid ? '#4fc1ff' : '#f44747'
						}`,
						borderRadius: '4px',
						padding: '12px',
						marginBottom: '16px'
					...
					>
						<div>
							...
							<span
								style={{
									color:
										validationSummary.totalErrors > 0 ? '#f44747' : '#4fc1ff'
								}}
							>
						...
						<div>
							...
							<span
								style={{
									color:
										validationSummary.totalWarnings > 0 ? '#ffcc02' : '#4fc1ff'
								}}
							>
						...
						{showSuggestions && (
							<div>
								...
								<span style={{ color: '#4fc1ff' }}>
									{validationSummary.totalSuggestions}
								</span>
							</div>
						)}
						<div>
							...
							<span style={{ color: '#4fc1ff' }}>
								{validationSummary.loadedComponents.length}/
								{validationSummary.loadedComponents.length +
									validationSummary.failedComponents.length}
							</span>
						</div>
					</div>
				</div>
	...
	return (
		...
		>
			<div style={{ display: 'flex', alignItems: 'flex-start', gap: '8px' }}>
				...
				<div style={{ flex: 1 }}>
					<div
						style={{ fontSize: '12px', color: '#cccccc', marginBottom: '2px' }}
					>
						{item.field}
					</div>
					<div
						style={{ fontSize: '11px', color: '#d4d4d4', lineHeight: '1.4' }}
					>
						{item.message}
					</div>
					{item.line && (
						<div
							style={{ fontSize: '10px', color: '#808080', marginTop: '2px' }}
						>
							Line {item.line}
		...
		>
			...
			>
				...
				<div style={{ marginBottom: '16px' }}>
					<div
						style={{ color: '#d4d4d4', fontSize: '12px', lineHeight: '1.5' }}
					>
						{error.message}
					</div>
