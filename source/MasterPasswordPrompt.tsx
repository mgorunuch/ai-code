...
export const MasterPasswordPrompt: React.FC<MasterPasswordPromptProps> = ({ 
      ...
      if (!globalOrchestrator) {
        ...
        try {
          ...
          try {
            ...
            if (credentialData.credentials && Object.keys(credentialData.credentials).length > 0) {
  ...
  const initializeCredentials = async () => {
    if (!globalOrchestrator || !masterPassword.trim() || attemptCount >= MAX_ATTEMPTS) return;
    ...
    try {
      ...
      if (hasExistingCredentials) {
        ...
        try {
          ...
          await globalOrchestrator.storeCredential('__password_test__', testValue);
          
          // Try to retrieve it back to confirm password is correct
          const retrieved = await globalOrchestrator.getCredential('__password_test__');
          
          if (retrieved !== testValue) {
            throw new Error('Password verification failed - credential mismatch');
          }
        } catch (verifyError) {
          const errorMsg = (verifyError as Error).message;
      ...
      if (errorMsg.includes('Invalid master password') || 
        ...
        if (newAttemptCount >= MAX_ATTEMPTS) {
          setError(`❌ Too many failed attempts (${MAX_ATTEMPTS}). Access locked.`);
          setIsLocked(true);
          ...
          if (hasExistingCredentials) {
            setError(`❌ Wrong password. ${MAX_ATTEMPTS - newAttemptCount} attempts remaining.`);
          } else {
            setError(`❌ Password setup failed. ${MAX_ATTEMPTS - newAttemptCount} attempts remaining.`);
          }
        }
      } else {
  ...
  return (
    <Box flexDirection="column" padding={1}>
      <Box marginBottom={1}>
        <Text bold color="cyan">🔐 Secure Credential Storage</Text>
      </Box>
      ...
      <Box marginBottom={1} flexDirection="column">
        {hasExistingCredentials ? (
          <>
            <Text color="gray">Enter your master password to unlock existing encrypted credentials.</Text>
            <Text color="yellow">You have stored API keys that need to be decrypted.</Text>
          </>
        ) : (
          <>
            <Text color="gray">Set up encrypted API key storage.</Text>
            <Text color="gray">Create a master password to secure your API keys between sessions.</Text>
          </>
        )}
      </Box>
      ...
      <Box marginBottom={1}>
        ...
        <Text color="yellow">{masterPassword.replace(/./g, '•') || '(empty)'}</Text>
      </Box>

      {attemptCount > 0 && !isLocked && (
        <Box marginBottom={1}>
          <Text color="orange">Attempts remaining: {MAX_ATTEMPTS - attemptCount}</Text>
        </Box>
      )}

      {isLocked ? (
        <Box marginBottom={1} flexDirection="column">
          <Text color="red">🔒 Access locked due to too many failed attempts</Text>
        ...
        <Box marginBottom={1} flexDirection="column">
          {hasExistingCredentials ? (
            <>
              <Text dimColor>Enter the correct master password and press Enter</Text>
              <Text dimColor>Press Escape to exit (credentials required for app usage)</Text>
            </>
          ) : (
            <>
