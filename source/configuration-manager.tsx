...
const TabPanel: React.FC<TabPanelProps> = ({ children, value, index }) => {
	return (
		<div
			style={{ display: value === index ? 'block' : 'none', padding: '16px 0' }}
		>
...
export const ConfigurationManager: React.FC<ConfigurationManagerProps> = ({
	...
	const [coreConfigManager, setCoreConfigManager] =
		useState<CoreConfigurationManager | null>(null);
	...
	const tabs: ConfigurationTab[] = [
		...
		},
		{
			...
			component: () => (
				<SecurityPanel context={configContext} onChange={handleConfigChange} />
			)
		},
		{
			id: 'import-export',
	...
	const handleSaveConfiguration = async () => {
		...
		try {
			...
			const updatedConfig = applyChanges(
				configContext.resolvedConfig,
				previewChanges
			);

			// Validate before saving
			const validationResult = await coreConfigManager.validateConfiguration(
				updatedConfig
			);
	...
	const determineConfigType = (
		changes: ConfigurationChange[]
	): 'orchestration' | 'user' | 'models' | 'security' | null => {
	...
	return (
		...
		>
			...
			<div style={{ marginBottom: '16px' }}>
				...
				>
					<div style={{ fontSize: '12px', color: '#808080' }}>
						<Text>
							{initializationError
								? `Init Error: ${initializationError}`
								: configContext.isLoading
								? 'Loading...'
								: configContext.hasUnsavedChanges
								? 'Unsaved changes'
								: coreConfigManager
								? 'Connected to core config system'
								: 'Initializing...'}
						</Text>
					</div>
					<div style={{ fontSize: '12px', color: '#808080' }}>
			...
			{configContext.validationErrors.length > 0 && (
				...
				>
					<h3 style={{ margin: '0 0 8px 0', color: '#f44747' }}>
						<Text>Configuration Issues</Text>
					</h3>
			...
			<div style={{ display: 'flex', flex: 1, gap: '16px' }}>
				...
				<div style={{ width: '200px', flexShrink: 0 }}>
					{tabs.map((tab) => (
						<button
							...
							style={{
								...
								marginBottom: '4px',
								backgroundColor:
									activeTab === tab.id ? '#264f78' : 'transparent',
					...
					<div style={{ marginTop: '16px' }}>
						<button
							...
							disabled={
								!configContext.hasUnsavedChanges || configContext.isLoading
							}
							style={{
								display: 'block',
								width: '100%',
								padding: '8px 12px',
								marginBottom: '8px',
								backgroundColor: configContext.hasUnsavedChanges
									? '#0e639c'
									: '#3c3c3c',
								color: configContext.hasUnsavedChanges ? '#ffffff' : '#808080',
								border: 'none',
								borderRadius: '4px',
								cursor: configContext.hasUnsavedChanges
									? 'pointer'
									: 'not-allowed',
								fontSize: '12px'
							}}
				...
				{showPreview && (
					...
					>
						<h3 style={{ margin: '0 0 12px 0', color: '#569cd6' }}>
							<Text>Configuration Preview</Text>
						</h3>
...
const AgentConfigurationPanel: React.FC<{
	...
	return (
		<div>
			...
			>
				<h2 style={{ color: '#569cd6', margin: 0 }}>
			...
			>
				...
				>
					<h3
						style={{ color: '#569cd6', margin: '0 0 8px 0', fontSize: '12px' }}
					>
						<Text>Agents</Text>
					</h3>
					{agentConfigs.map((agent, index) => (
						<div
							...
							style={{
								...
								marginBottom: '4px',
								backgroundColor:
									selectedAgent === (agent.id || index.toString())
										? '#264f78'
										: 'transparent',
								border: '1px solid #3c3c3c',
								borderRadius: '3px',
							...
							configFile={
								selectedAgent === 'new'
									...
									: {
											id: selectedAgent,
											name:
												agentConfigs.find((a) => a.id === selectedAgent)
													?.name || 'Agent',
											description:
												agentConfigs.find((a) => a.id === selectedAgent)
													?.description || '',
											filePath: `.ai-code/agents/${selectedAgent}.agent.ts`,
											content: generateAgentFileContent(
												agentConfigs.find((a) => a.id === selectedAgent)
											),
...
const ModelConfigurationPanel: React.FC<{
	...
	const [modelConfig, setModelConfig] = useState(
		context.resolvedConfig?.modelSelection || {}
	);
	...
	return (
		<div>
			<h2 style={{ color: '#569cd6', marginBottom: '16px' }}>
				<Text>Model Selection Configuration</Text>
			</h2>
			...
			>
				<div>
					...
					>
						...
						<option value="claude-3-haiku">
							<Text>Claude 3 Haiku</Text>
						</option>
						<option value="claude-3-opus">
							<Text>Claude 3 Opus</Text>
						</option>
						<option value="gpt-4">
							<Text>GPT-4</Text>
						</option>
						<option value="gpt-3.5-turbo">
							<Text>GPT-3.5 Turbo</Text>
						</option>
					</select>
				</div>
				...
				<div>
					<h3
						style={{ color: '#569cd6', marginBottom: '12px', fontSize: '14px' }}
					>
						<Text>Selection Strategy</Text>
					</h3>
					<select
						value={modelConfig.selectionStrategy || 'balanced'}
						onChange={(e) =>
							updateModelConfig('selectionStrategy', e.target.value)
						}
						style={{
							width: '100%',
							padding: '6px',
					...
					>
						<option value="cost-optimized">
							<Text>Cost Optimized</Text>
						</option>
						<option value="performance-optimized">
...
const CredentialsPanel: React.FC<{
	...
	<CredentialManager context={context} onChange={onChange} />
);
...
const SecurityPanel: React.FC<{
	...
	<div>
		<h2 style={{ color: '#569cd6', marginBottom: '16px' }}>
			<Text>Security & Access Patterns</Text>
		</h2>
		...
		>
			<div>
				...
				>
					<input
						type="checkbox"
						checked={
							context.resolvedConfig?.defaultPermissions
								?.requireExplicitToolGrants || false
						}
						onChange={(e) => {
							const change: ConfigurationChange = {
					...
					<span style={{ color: '#cccccc', fontSize: '12px' }}>
						<Text>Require Explicit Tool Grants</Text>
					</span>
				</label>
			</div>
			...
			<div>
				...
				>
					...
					/>
					<span style={{ color: '#cccccc', fontSize: '12px' }}>
						<Text>Enable Access Pattern Validation</Text>
					</span>
				</label>
			</div>
		</div>
...
const ImportExportPanel: React.FC<{
	...
	<ConfigurationImportExport context={context} onChange={onChange} />
);
