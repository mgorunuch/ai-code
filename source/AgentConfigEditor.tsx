...
export const AgentConfigEditor: React.FC<AgentConfigEditorProps> = ({
  ...
  const [content, setContent] = useState(configFile?.content || getDefaultAgentTemplate());
  const [mode, setMode] = useState<'visual' | 'code'>('visual');
  const [agentForm, setAgentForm] = useState<AgentCapabilityForm>(parseAgentFromContent(content));
  ...
  const handleFormChange = useCallback((newForm: AgentCapabilityForm) => {
    ...
    <div style={{ 
      ...
      <div style={{ 
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          {/* Mode Toggle */}
          <div style={{ display: 'flex', border: '1px solid #3c3c3c', borderRadius: '4px' }}>
            <button
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <button
            ...
            style={{
              padding: '4px 8px',
              fontSize: '11px',
              backgroundColor: validationResult.errors.length === 0 ? '#0e639c' : '#3c3c3c',
              color: validationResult.errors.length === 0 ? '#ffffff' : '#808080',
              border: 'none',
              borderRadius: '3px',
              cursor: validationResult.errors.length === 0 ? 'pointer' : 'not-allowed'
            }}
          >
            Save
          </button>
  ...
  return (
    <div style={{ 
      ...
      <section style={{ marginBottom: '24px' }}>
        <h4 style={{ color: '#569cd6', marginBottom: '12px' }}>Basic Information</h4>
    ...
    <div style={{ 
      ...
      <div style={{ flex: 1, position: 'relative' }}>
        ...
        {editorSettings.syntaxHighlighting && (
          <SyntaxHighlightOverlay content={content} editorSettings={editorSettings} />
        )}
...
const ToolsEditor: React.FC<{
  ...
  return (
    <section style={{ marginBottom: '24px' }}>
      <h4 style={{ color: '#569cd6', marginBottom: '12px' }}>Tools Configuration</h4>
...
const EndpointsEditor: React.FC<{
  ...
  return (
    <section style={{ marginBottom: '24px' }}>
      <h4 style={{ color: '#569cd6', marginBottom: '12px' }}>Endpoints Configuration</h4>
      <div style={{ color: '#808080', fontSize: '12px' }}>
...
const ValidationPanel: React.FC<{ validationResult: ValidationResult }> = ({ validationResult }) => {
  return (
    <div>
...
export const ${form.id.replace(/-/g, '_')}: AgentCapability = {
  ...
  endpoints: [
${form.endpoints.map(ep => `    { name: '${ep.name}', description: '${ep.description}' }`).join(',\n')}
  ]
};
`;
}
