...
import type { OperationRequest, OperationResponse, OperationType } from './core/types.js';
...
export const OperationMonitor: React.FC<OperationMonitorProps> = ({ 
  ...
  const { activeOperations, recentResponses, isConnected, stats } = useAgentCommunication();
  ...
  useInput((input, key) => {
    ...
    if (key.downArrow && selectedIndex < Math.min(displayItems.length - 1, maxOperations - 1)) {
      setSelectedIndex(selectedIndex + 1);
    }
  ...
  const filteredCompletedOps = filterByType 
    ? completedOps.filter(resp => {
        ...
        const request = Array.from(activeOperations.values()).find(req => req.requestId === resp.requestId);
        return request?.type === filterByType;
      })
    : completedOps;

  // Create display items based on view
  const getDisplayItems = (): Array<{ type: 'active' | 'completed'; data: OperationRequest | OperationResponse }> => {
    const items: Array<{ type: 'active' | 'completed'; data: OperationRequest | OperationResponse }> = [];
  ...
  const getOperationTypeColor = (type: OperationType): string => {
    switch (type) {
      ...
      case 'write_file': return 'green';
      case 'edit_file': return 'yellow';
      case 'delete_file': return 'red';
      case 'create_directory': return 'magenta';
      case 'question': return 'cyan';
      case 'validate': return 'orange';
      case 'transform': return 'purple';
      default: return 'white';
    }
  };

  const getStatusColor = (item: { type: 'active' | 'completed'; data: OperationRequest | OperationResponse }): string => {
  ...
  if (!isConnected) {
    return (
      <Box flexDirection="column" borderStyle="round" borderColor="red" padding={1}>
        <Text color="red" bold>Operation Monitor Unavailable</Text>
    ...
    <Box flexDirection="column" borderStyle="round" borderColor="blue" padding={1}>
      <Box flexDirection="row" justifyContent="space-between">
        ...
        <Box flexDirection="row" gap={1}>
          ...
          <Text color={view === 'completed' ? 'cyan' : 'gray'}>C:Completed</Text>
      ...
      {displayItems.length === 0 ? (
        ...
        <Box marginTop={1} flexDirection="column">
          ...
          {displayItems.map((item, index) => {
            ...
            return (
              <Box key={isActive ? request.requestId : response.requestId} flexDirection="row" justifyContent="space-between">
                <Box width={10}>
                  <Text 
                  ...
                  >
                    {isSelected ? '→ ' : '  '}
                    {isActive ? '⏳ RUNNING' : (response.success ? '✓ SUCCESS' : '✗ FAILED')}
                  </Text>
                </Box>
                
                <Box width={12}>
                  <Text color={getOperationTypeColor(isActive ? request.type : 'read_file')}>
                ...
                <Box width={15}>
                  <Text wrap="truncate">
                    {isActive ? (request.requestingAgent || 'system') : response.handledBy}
                  </Text>
                </Box>
      ...
      {showDetails && selectedItem && (
        <Box marginTop={1} borderStyle="single" borderColor="gray" padding={1}>
          <Text color="cyan" bold>Operation Details</Text>
          
          {selectedItem.type === 'active' ? (
            <ActiveOperationDetails operation={selectedItem.data as OperationRequest} />
          ) : (
            <CompletedOperationDetails response={selectedItem.data as OperationResponse} />
          )}
      ...
      {(filteredActiveOps.length + filteredCompletedOps.length) > maxOperations && (
        <Box marginTop={1}>
          <Text dimColor>
            ... and {(filteredActiveOps.length + filteredCompletedOps.length) - maxOperations} more operations
          </Text>
        </Box>
      )}
    </Box>
...
const ActiveOperationDetails: React.FC<{ operation: OperationRequest }> = ({ operation }) => (
  <Box flexDirection="column">
    <Text dimColor>Request ID: {operation.requestId}</Text>
    <Text>Type: <Text color="yellow">{operation.type}</Text></Text>
    <Text>Target: <Text color="cyan">{operation.filePath || 'system'}</Text></Text>
    <Text>Requesting Agent: <Text color="green">{operation.requestingAgent || 'system'}</Text></Text>
    
    {operation.payload && (
      <Box marginTop={1} flexDirection="column">
        <Text color="magenta" bold>Payload:</Text>
...
const CompletedOperationDetails: React.FC<{ response: OperationResponse }> = ({ response }) => (
  <Box flexDirection="column">
    <Text dimColor>Request ID: {response.requestId}</Text>
    <Text>Status: <Text color={response.success ? 'green' : 'red'}>{response.success ? 'SUCCESS' : 'FAILED'}</Text></Text>
    <Text>Handled By: <Text color="green">{response.handledBy}</Text></Text>
    ...
    {response.metadata && (
      <Box marginTop={1} flexDirection="column">
        <Text color="blue" bold>Metadata:</Text>
...
export const OperationStats: React.FC<OperationStatsProps> = ({ showDetailed = false }) => {
  ...
  const successRate = recentResponses.length > 0 ? (successCount / recentResponses.length) * 100 : 0;
  ...
  return (
    <Box flexDirection="column" borderStyle="single" borderColor="blue" padding={1}>
      <Text color="blue" bold>Operation Statistics</Text>
      
      <Box flexDirection="row" gap={4} marginTop={1}>
        <Box flexDirection="column">
          <Text color="yellow" bold>Active Operations</Text>
          <Text>{activeOperations.size}</Text>
        </Box>
        
        <Box flexDirection="column">
          <Text color="green" bold>Successful</Text>
        ...
        <Box flexDirection="column">
          <Text color="cyan" bold>Success Rate</Text>
          <Text>{successRate.toFixed(1)}%</Text>
        </Box>
      </Box>
      
      <Box marginTop={1}>
        <Text dimColor>Total Requests: {stats.totalRequests} | Total Responses: {stats.totalResponses}</Text>
      </Box>
    </Box>
  );
};