...
import type { ModelSelectionResult, ModelSelectionCriteria, AIModel } from './core/types.js';
...
export const ModelSelectionDisplay: React.FC<ModelSelectionDisplayProps> = ({ 
  ...
  const { modelSelections, isConnected, orchestrator } = useAgentCommunication();
  ...
  useInput((input, key) => {
    ...
    if (key.downArrow && selectedIndex < Math.min(modelSelections.length - 1, maxSelections - 1)) {
      setSelectedIndex(selectedIndex + 1);
    }
  ...
  if (!isConnected || !orchestrator) {
    return (
      <Box flexDirection="column" borderStyle="round" borderColor="red" padding={1}>
        <Text color="red" bold>Model Selection Display Unavailable</Text>
  ...
  return (
    <Box flexDirection="column" borderStyle="round" borderColor="purple" padding={1}>
      <Text color="purple" bold>Model Selection Monitor</Text>
          ...
          {displaySelections.map((selection, index) => {
            ...
            return (
              <Box key={index} flexDirection="row" justifyContent="space-between">
                <Box width={20}>
                  <Text 
                    color={isSelected ? 'cyan' : getModelColor(selection.result.selectedModel)}
                    bold={isSelected}
                    wrap="truncate"
                  >
                    {isSelected ? '→ ' : '  '}{selection.result.selectedModel}
                  </Text>
                </Box>
                
                <Box width={15}>
                  <Text color="yellow">
                    {selection.criteria.operationType}
                  </Text>
                </Box>
                ...
                <Box width={10}>
                  <Text dimColor>
                    {selection.result.estimatedCost ? `$${selection.result.estimatedCost.toFixed(4)}` : 'N/A'}
                  </Text>
                </Box>
              </Box>
            );
      ...
      {modelSelections.length > maxSelections && (
        <Box marginTop={1}>
          <Text dimColor>... and {modelSelections.length - maxSelections} more selections</Text>
        </Box>
      )}
...
const ModelSelectionStats: React.FC<{
  selections: Array<{ criteria: ModelSelectionCriteria; result: ModelSelectionResult }>;
  config: any;
}> = ({ selections, config }) => {
    ...
    <Box flexDirection="column" marginTop={1} marginBottom={1}>
      <Box flexDirection="row" gap={3}>
        <Box flexDirection="column">
          <Text color="cyan" bold>Total Selections</Text>
          <Text>{selections.length}</Text>
        </Box>
        
        <Box flexDirection="column">
          <Text color="green" bold>Avg Confidence</Text>
          <Text>{(avgConfidence * 100).toFixed(1)}%</Text>
        </Box>
        
        <Box flexDirection="column">
          <Text color="yellow" bold>Total Cost</Text>
          <Text>${totalCost.toFixed(4)}</Text>
        </Box>
        
        <Box flexDirection="column">
          <Text color="purple" bold>Most Used</Text>
          <Text>{mostUsedModel ? mostUsedModel[0] : 'N/A'}</Text>
        </Box>
      </Box>
      
      <Box marginTop={1}>
        <Text dimColor>Auto Mode: {config.autoModeConfig?.enabled ? 'ON' : 'OFF'}</Text>
        <Text dimColor> | Available Models: {config.availableModels?.length || 0}</Text>
      </Box>
    </Box>
  );
};
...
const ModelSelectionDetails: React.FC<{
  ...
  return (
    <Box flexDirection="column">
      <Text color="purple" bold>Model Selection Details</Text>
      
      <Box marginTop={1} flexDirection="column">
        <Text color="cyan" bold>Selected Model:</Text>
        <Text>{result.selectedModel}</Text>
        <Text dimColor>Reason: {result.reason}</Text>
        <Text dimColor>Confidence: {(result.confidence * 100).toFixed(1)}%</Text>
        {result.estimatedCost && (
          <Text dimColor>Estimated Cost: ${result.estimatedCost.toFixed(4)}</Text>
        )}
      </Box>
      
      <Box marginTop={1} flexDirection="column">
        <Text color="yellow" bold>Selection Criteria:</Text>
        <Text>Operation: {criteria.operationType}</Text>
        {criteria.agentId && <Text>Agent: {criteria.agentId}</Text>}
        {criteria.complexity && <Text>Complexity: {criteria.complexity}/10</Text>}
        {criteria.contextLength && <Text>Context Length: {criteria.contextLength}</Text>}
        {criteria.priority && <Text>Priority: {criteria.priority}/10</Text>}
        {criteria.maxCost && <Text>Max Cost: ${criteria.maxCost.toFixed(4)}</Text>}
      </Box>
      
      {result.alternatives && result.alternatives.length > 0 && (
        <Box marginTop={1} flexDirection="column">
          <Text color="green" bold>Alternative Models Considered:</Text>
...
export const ModelConfigDisplay: React.FC<ModelConfigDisplayProps> = ({ compact = false }) => {
  ...
  return (
    <Box flexDirection="column" borderStyle="single" borderColor="purple" padding={1}>
      <Text color="purple" bold>Model Configuration</Text>
      
      <Box marginTop={1} flexDirection="column">
        <Text color="cyan">Auto Mode: {config.autoModeConfig?.enabled ? 'ENABLED' : 'DISABLED'}</Text>
      ...
      {config.availableModels && config.availableModels.length > 0 && (
        <Box marginTop={1} flexDirection="column">
          <Text color="green" bold>Available Models:</Text>
          {config.availableModels.map((model, index) => (
            <Box key={model.model} flexDirection="row">
              <Text 
                color={index === selectedModelIndex ? 'cyan' : getModelColor(model.model)}
                bold={index === selectedModelIndex}
              >
                {index === selectedModelIndex ? '→ ' : '  '}{model.name}
              </Text>
              <Text dimColor> - {model.available ? 'Available' : 'Unavailable'}</Text>
            </Box>
          ))}
        </Box>
      )}
